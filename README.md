...
| [generate-supervisor-metadata-sql](skills/generate-supervisor-metadata-sql/SKILL.md) | Generates SQL that merges a boolean flag such as `is_supervisor` into `app_employee.metadata` from a roster spreadsheet, preserving existing JSON keys. Emits a preflight query and a rollback file. | Python 3, [`openpyxl`](https://pypi.org/project/openpyxl/) for `.xlsx` input |
