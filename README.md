...
| [gtm](skills/gtm/SKILL.md) | Release workflow — bumps the version, creates a release branch, publishes a GitHub release with auto-generated notes, and opens release PRs. Auto-detects gitflow vs trunk-based branching and reads the version from a `version` file, `setup.py`, or `pyproject.toml`. | [GitHub CLI (`gh`)](https://cli.github.com) installed and authenticated, a `version` file / `setup.py` with `version=` / `pyproject.toml` with `version =` at the project root |
